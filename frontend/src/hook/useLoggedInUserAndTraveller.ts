...
import {getLoggedInTraveller, getLoggedInUserName, login, logoutUser, updateUser} from "../apiCalls";
...
export default function useLoggedInUserAndTraveller() {
    ...
    //
    function updateLoggedInTraveller(id: string,traveller:Traveller){
        updateUser(id, traveller)
            .then(data => setLoggedInTraveller(data))
    }



    return {loggedInTraveller, logout, loginUser, username, updateLoggedInTraveller}
}